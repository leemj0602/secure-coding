...
exports.processDesignSubmission = (req, res, next) => {
    ...
    var rDesignDescription = designDescription.replace(reg1, "");
    rDesignDescription = rDesignDescription.replace(reg2, "");
    ...
    fileDataManager.uploadFile(file, async function (error, result) {
        ...
        if (error) {
            ...
            try {
                let result = await fileDataManager.createFileData(imageURL, publicId, userId, rDesignTitle, rDesignDescription);
...
exports.processUpdateOneUser = async (req, res, next) => {
    ...
    try {
        ...
        console.log("user_id " + recordId + "'s role_id changed to " + newRoleId);
        console.log(results);
...
exports.processUpdateOneDesign = async (req, res, next) => {
    ...
    try {
        results = await userManager.updateDesign(fileId, rDesignTitle, rDesignDescription);
        console.log(results);
        return res.status(200).json({ message: 'Completed update' });
    } catch (error) {
        console.log('processUpdateOneUser method : catch block section code is running');
